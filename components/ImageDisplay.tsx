...
import { Loader2, AlertTriangle, Download, Clock, Database, ArrowUp } from 'lucide-react';
...
export const ImageDisplay: React.FC<ImageDisplayProps> = ({ result, label, color, onUseAsInput }) => {
      ...
      <div className={`flex-grow relative border-4 border-black bg-white rounded-lg overflow-hidden min-h-[300px] flex items-center justify-center group`}>
        ...
        {result.loading ? (
          ...
          <>
            ...
            <div className="absolute bottom-4 right-4 flex gap-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-200">
              {onUseAsInput && (
                <button
                  onClick={() => result.imageUrl && onUseAsInput(result.imageUrl)}
                  title="Use as Input"
                  className="bg-archi-black text-white p-2 rounded-full hover:scale-110 shadow-archi-sm hover:bg-archi-yellow hover:text-black transition-all border-2 border-transparent hover:border-black"
                >
                  <ArrowUp size={20} />
                </button>
              )}
              <a 
                ...
                title="Download"
                className="bg-archi-black text-white p-2 rounded-full hover:scale-110 shadow-archi-sm hover:bg-archi-red transition-all border-2 border-transparent hover:border-black"
              >
                <Download size={20} />
              </a>
            </div>
